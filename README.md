...
| `ddskk-jev-timeout` | 1.0 | HTTP のタイムアウト秒数 |
...
- **失敗が続くと自動停止します。** `ddskk-jev-max-consecutive-failures` 回連続で失敗すると問い合わせを止め、メッセージを表示します。`M-x ddskk-jev-reset` で再開できます。
- **課金が発生します。** Jev の価格は AI Gateway のモデルページを参照してください。同じ文脈と候補の組み合わせはキャッシュし、同一変換内での重複問い合わせは行いません。
